...
def calculate_framewise_displacement(
    mask_file, csv_file, output_csv=None, verbose=False
):
    if verbose:
    ...
    results.append({"timepoint": 0, "mean_fd": 0.0, "max_fd": 0.0})
    ...
    for i in tqdm(range(len(transforms) - 1)):
        ...
        results.append({"timepoint": i + 1, "mean_fd": mean_fd, "max_fd": max_fd})
    ...
    if output_csv:
        with open(output_csv, "w", newline="") as f:
            ...
            for res in results:
                writer.writerow([res["timepoint"], res["mean_fd"], res["max_fd"]])
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate Framewise Displacement from moco.csv and mask using point tracking."
    )
    ...
    parser.add_argument(
        "--output_csv", help="Optional output CSV for FD values", default=None
    )
    parser.add_argument(
        "--verbose", help="Optional verbose output", action="store_true"
    )
    ...
    calculate_framewise_displacement(
        args.mask_file, args.csv_file, args.output_csv, args.verbose
    )
